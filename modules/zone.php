...
{	
	...
	function zone_new()
	{
		...
		if($f[zone_style])
		{
			$z = $this->db->getsql("SELECT * FROM adrev_ad_types WHERE id='$f[zone_style]'");
		...
		if($f[zone_style] && $f[zone_name])
		{
			...
			if($z[0][id] && count($t) > 0)
			{
				...
				$this->output->redirect(lib_lang("Creating your zone..."), "index.php?section=zone", 1);
				exit;
			}
		}
	...
	function zone_clone()
	{
		...
		$tpl->parse("main");
		$this->output->title = "[".stripslashes($z[0][name])."] : " . lib_lang("Create Ad Type");
	...
	function zone_ads()
	{
		...
		if($_REQUEST[c] == "delete" && $f[adid])
		{
			...
			//if(!$st[0][id])
			//{
				$this->db->getsql("DELETE FROM adrev_ads WHERE id='$f[adid]'");
				$this->db->getsql("DELETE FROM adrev_keyword_map WHERE adid='$f[adid]'");
				$this->db->getsql("DELETE FROM adrev_traffic WHERE adid='$f[adid]'");
		...
		if($_GET[c] == "default" && $f[adid])
			$this->db->getsql("UPDATE adrev_zones SET default_ad='$f[adid]' WHERE id='$f[id]'");
		if($_GET[c] == "activate" && $f[adid])
			$this->db->getsql("UPDATE adrev_ads SET status='1' WHERE id='$f[adid]'");
		if($_GET[c] == "deactivate" && $f[adid])
			$this->db->getsql("UPDATE adrev_ads SET status='-1' WHERE id='$f[adid]'");			
		if($_GET[c] == "del_default" && $f[id])
			$this->db->getsql("UPDATE adrev_zones SET default_ad='0' WHERE id='$f[id]'");
		...
		$extra = "";
		if($f[id])
			$sqlextra[] = "a.zone='$f[id]'";
		if($f[email])
			$sqlextra[] = "b.email='$f[email]'";
		if($f[status] <> "all" && $f[status])
			$sqlextra[] = "a.status='$f[status]'";
		if(count($sqlextra) > 0)
		...
		if($z[0][default_ad])
		{
			$ad = $this->db->getsql("SELECT id,zid,title FROM adrev_ads WHERE id='".$z[0][default_ad]."'");
			$title = $ad[0][title];
			$id = $ad[0][id];
			$zid = $ad[0][zid];
	...
	function zone_list()
	{
		...
		if(count($zones) > 0)
		{
			foreach($zones as $rec)
			{
				...
				$a = $this->db->getsql("SELECT count(*) as num FROM adrev_ads WHERE zone='$rec[id]'");
				...
				$stats = $this->db->getsql("SELECT sum(a.clicks) as clicks, sum(a.impressions) as impressions, 
											...
											WHERE a.adid=b.id AND b.zone='{$rec['id']}' 
											AND a.date BETWEEN '{$sd}' AND '{$ed}'");
				...
				$a = $this->db->getsql("SELECT count(*) as num FROM adrev_ads WHERE zone='$rec[id]' AND status='2'");
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage Zones");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;
	...
	function zone_delete()
	{
		...
		$ads = $this->db->getsql("SELECT id FROM adrev_ads WHERE zone='$f[id]' LIMIT 1");
		if($ads[0][id])
		{
		...
		$this->db->getsql("DELETE FROM adrev_zones WHERE id='$f[id]'");
	...
	function zone_define()
	{
		...
		$z = $this->db->getsql("SELECT a.*,b.name as adtypename FROM adrev_zones a, adrev_ad_types b
										WHERE a.style=b.id AND a.id='$f[id]'");
		if($f[name])
		{
			$i = array();
		...
		if($f[id])
			$this->output->title = lib_lang("Edit Zone") . " [" . stripslashes($f[name]) . "]";
		else
			$this->output->title = lib_lang("Create new Zone");
	...
	function zone_format()
	{
		...
		$style = $z[0][style];
		$s = $this->db->getsql("SELECT * FROM adrev_ad_types WHERE id='$style'");
		...
		$ad = $this->db->getsql("SELECT zid FROM adrev_ads WHERE zone='$f[id]' LIMIT 1");
		if(!$ad[0][zid])
		{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage") . " [".stripslashes($z[0][name])."] : " . lib_lang("Format");	
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();			
		exit;			
	...
	function zone_layout()
	{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage") . " [".stripslashes($z[0][name])."] : " . lib_lang("Layout");	
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();			
		exit;			
	...
	function zone_daypart()
	{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage") . "[".stripslashes($z[0][name])."] : " . lib_lang("Dayparting");	
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();			
		exit;		
	...
	function zone_code()
	{
		...
		$tpl->parse("main");
		$this->output->title = "@@Manage@@ [".stripslashes($z[0][name])."] : @@Ad Codes@@";	
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();			
		exit;		
	...
	function zone_keywords()
	{
		...
		if($f[submit])
		{
			...
			if(count($k) > 0)
			{
				foreach($k as $keyword)
				{
					...
					$l = $this->db->getsql("SELECT id FROM adrev_keywords WHERE keyword='$key'");
					if(!$l[0][id])
					{
						// Add the keyword
						$i = array();
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage") . " [".stripslashes($z[0][name])."] : " . lib_lang("Keywords");	
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();			
		exit;		
