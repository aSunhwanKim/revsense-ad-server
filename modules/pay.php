...
class pay extends main
{
	...
	function approve()
	{
		...
		$p = $this->db->getsql("SELECT * FROM adrev_tokens WHERE token='$token' AND status='Pending'");
		if(!$p[0][id])
		{
		...
		else
		{
			...
			$id = $p[0][id];
			$this->db->getsql("UPDATE adrev_tokens SET status='Completed' WHERE id='$id'");
	...
	function reject()
	{
		...
		{			
			...
			$id = $p[0][id];
			$this->db->getsql("UPDATE adrev_tokens SET status='Rejected' WHERE id='$id'");
	...
	function success()
	{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Thank You");
	...
	function failure()
	{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("We're sorry");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();	
		exit;		
