...
class useradmin extends main
{
	function _default()
	{
		...
		if($f[search])
		{
			...
			$users = $this->db->getsql("SELECT * FROM adrev_users
											WHERE admin IN (1,3) AND email LIKE '%$f[search]%' OR name LIKE '%$f[search]%'
		...
		$tpl->parse("users");
		$this->output->title = lib_lang("Manage Users");
	...
	function view()
	{
		...
		$b = $this->db->getsql("SELECT sum(amount) as spend FROM adrev_traffic WHERE userid='$f[id]'");
		$spend = $b[0][spend];

		// Grab payment history summary
		$h = $this->db->getsql("SELECT sum(amount) as paid FROM adrev_payments WHERE userid='$f[id]'");		
		$paid = $h[0][paid];
		...
		$ts = time();
		$this->db->getsql("UPDATE adrev_users SET balance='$balance',balance_update='$ts' WHERE id='$f[id]'");

		// Grab Payment History records
		$h = $this->db->getsql("SELECT * FROM adrev_payments WHERE userid='$f[id]'");
		if(count($h) > 0)
		{
			foreach($h as $rec)
			{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage Users");
		$this->output->content = $tpl->text("users") . "<br>" . $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;		
	...
	function deleteu()
	{
	  // Delete an ad
		...
		if($_REQUEST[c] == "delete" && $f[id]!= 1)
		{
				...
				$this->db->getsql("DELETE FROM adrev_users WHERE id='$f[id]'");
				$this->db->getsql("DELETE FROM adrev_ads WHERE userid='$f[id]'");
				$this->db->getsql("DELETE FROM adrev_traffic WHERE userid='$f[id]'");
