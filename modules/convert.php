...
class convert extends main
{
	...
	function _default()
	{
		...
		{
			// Get the ad info
			$adinfo = $this->db->getsql("SELECT id,userid,zone FROM adrev_ads WHERE id='$k[id]'");
			$ad = $adinfo[0];
			
			// Get the zone info
			$zone = $this->db->getsql("SELECT rate,rate_type,aff_percent FROM adrev_zones WHERE id='$ad[zone]'");
			$z = $zone[0];
			...
			{	
				...
				$t = $this->db->getsql("SELECT * FROM adrev_traffic 
										WHERE adid='$k[id]' AND date='$date' AND keywordid='$k[kid]'");				
				if(!$t[0][id])
				{
				...
				else
				{
					...
					$this->db->getsql("UPDATE adrev_traffic SET orders=orders+1,amount=amount+$amount WHERE id='$id'");
				}
