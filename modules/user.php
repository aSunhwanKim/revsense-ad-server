...
class user extends main
{
	...
	function forgot()
	{
		...
		if($f[email])
		{
			$rec = $this->db->getsql("SELECT * FROM adrev_users WHERE email='$f[email]'");
			if($rec[0][email])
			{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Retrieve your Password");
	...
	function register()
	{
		...
		if($f[email] && $f[name] && $f[password] && $f[postalcode] && $f[city] && $f[state] && $f[country])
		{
			...
			$em = $this->db->getsql("SELECT id FROM adrev_users WHERE email='$f[email]'");
			if($em[0][id])
		...
		$this->output->title = $adname;
		$this->output->content = $form->generate("post", lib_lang("Register"));
		$this->output->display();
		$this->output->printpage();	
		exit;
	}	
	...
	function activate()
	{
		...
		if($zid)
		{
			$z = $this->db->getsql("SELECT id FROM adrev_users WHERE zid='$zid'");
			if($z[0][id])
			{
				$this->db->getsql("UPDATE adrev_users SET status='1' WHERE zid='$zid'");
	...
	function login()
	{
		...
		if($f[email] && $f[password])
		{
			...
			$rec = $this->db->getsql("SELECT * FROM adrev_users WHERE email='$f[email]' AND password='$f[password]' AND status='1'");
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Login");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();	
		exit;
