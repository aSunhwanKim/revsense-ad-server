...
#		$zone = lib_cache_get("zone", $ad[zone]);
		if(!$zone)
		{
			$z = $this->db->getsql("SELECT * FROM adrev_zones WHERE id='$ad[zone]'");
			$zone = $z[0];
		...
		if($this->keyid)
		{
			$k = $this->db->getsql("SELECT url FROM adrev_keyword_map WHERE keywordid='$this->keyid' AND adid='$ad[id]' LIMIT 1");
			if($k[0][url])
				$hash[URL] = $k[0][url];
		}
