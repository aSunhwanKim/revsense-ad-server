...
class payment extends main
{
	...
	function form()
	{
		...
		$this->output->title = lib_lang("Enter your Credit Card Information");
	...
	function process()
	{
		...
		$inv = $this->db->getsql("SELECT id FROM adrev_tokens WHERE token='$tok'");
		$invoice = $inv[0][id];

		// Get the last insert id
		$last = $this->db->getsql("SELECT id FROM adrev_tokens WHERE token='$tok'");
		$invoice = $last[0][id];
	...
	function external()
	{
		...
		$token = $_GET[t];
		$t = $this->db->getsql("SELECT * FROM adrev_tokens WHERE token='$token'");
