...
class ads extends main
{
	...
	function renew()
	{
		...
		$a = $this->db->getsql("SELECT * FROM adrev_ads WHERE zid='$f[id]' AND userid='$uid'");
		$ad = $a[0];
		if($ad[id])
		{
			$this->db->getsql("UPDATE adrev_ads SET expires=0, total_units=0, status=1 WHERE id='$ad[id]'"); 
	...
	function conversioncode()
	{
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Ad Conversion Code");
	...
	function adzone()
	{
		...
		$z = $this->db->getsql("SELECT a.*,b.name as typename 
									...
									WHERE a.style=b.id AND a.id='$id'");
		$zone = $z[0];
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Create New Ad in") . ": <font color=black>$zone[name]</font>" ;
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;
	...
	function listing()
	{
		...
		if($_REQUEST[c] == "pause" && $f[id])
		{
			...
			$this->db->getsql("UPDATE adrev_ads SET status='3' WHERE zid='$f[id]' AND userid='$uid'");			
		...
		elseif($_REQUEST[c] == "activate" && $f[id])
		{
			...
			if($check[ad_sort]=='bid' AND $check[keywords_enable]!=1)
			{
			   ...
			   {
			     $this->db->getsql("UPDATE adrev_ads SET status='1' WHERE zid='$f[id]' AND userid='$uid'");
		...
		$ads = $this->db->getsql("SELECT a.*,b.name, b.rate, b.rtype, rate_type, ad_sort, a.status, a.zone, keywords_enable 
									FROM adrev_ads a, adrev_zones b
									WHERE a.userid='$uid' AND a.zone=b.id
									ORDER BY a.id DESC");
		if(count($ads) > 0)
		{
			foreach($ads as $rec)
			{
				...
				$stats = $this->db->getsql("SELECT count(id) as num, sum(clicks) as clicks, 
												...
												sum(amount) as spend
											FROM adrev_traffic
											WHERE adid='$rec[id]' AND userid='$uid' 
				...
				if($rec[rtype] == 2)
				{
					...
					$position = $this->db->getsql("SELECT zid,bid FROM adrev_ads WHERE zone='$rec[zone]'
																				 AND bid >= '$rec[rate]' 
																				 AND status != '-2'
																				 AND status != '-1'
																				 AND status != '3'
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("My Ads");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;			
	...
	function edit()
	{
		...
		{
			$xad = $this->db->getsql("SELECT userid FROM adrev_ads WHERE zid='$f[id]'");
			$uid = $xad[0][userid];
		}		
		...
		if($f[submit] && count($f[data]) > 0)
		{
			...
			$d = $this->db->getsql("SELECT * FROM adrev_ads WHERE zid='$f[id]' AND userid='$uid'");
			if(!$f[zone])
				$f[zone] = $d[0][zone];
			if($f[zone])
			{
				$z = $this->db->getsql("SELECT * FROM adrev_zones WHERE id='$f[zone]'");
				$zone = $z[0];
			...
			if(!$errormsg)
			{
				...
				{	
			        ...
			        if($check[ad_sort]=='bid' AND $check[keywords_enable]!=1)
					{
					   ...
					   if($i[bid] < $zone[rate])
					    {
					   $this->db->getsql("UPDATE adrev_ads SET status='3', bid='$i[bid]' WHERE zid='$f[id]' AND userid='$uid'");
		...
		{
			$zid = $f[id];
			$d = $this->db->getsql("SELECT * FROM adrev_ads WHERE zid='$zid'");
			$f = $d[0];
			$f[id] = $zid;
			$f[data] = $f;
		}
		...
		if($f[zone])
		{
			...
			{	
				...
				else
				{
					...
					$tpl->assign("NAME", "<b>" . str_replace(array("-", " "), "&nbsp;", $val[name]) . "</b>");
					if($val[max_length] < 80)
						$tpl->assign("FIELD", "<input type=\"text\" name=\"f[data][$val[type]]\" value=\"$value\" size=\"$size\" maxlength=\"$val[max_length]\">");
					else
						$tpl->assign("FIELD", "<textarea name=\"f[data][$val[type]]\" rows=2 cols=50 wrap=virtual>$value</textarea>");
					$tpl->parse("main.fields");
				}
		...
		else
		{
			...
			if($_SESSION[user][admin] == 3)
				...
				$zlist = $this->db->getsql("SELECT * FROM adrev_zones WHERE status=1 ORDER BY name");
		...
		if($f[id])
			$this->output->title = lib_lang("Edit Ad");
		else
			$this->output->title = lib_lang("Create new Ad");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;		
	...
	function keyword_stats()
	{
		...
		// Grab the ad info
		$ad = $this->db->getsql("SELECT * FROM adrev_ads WHERE zid='$f[id]' AND userid='$uid'");
		$id = $ad[0][id];
		$zoneid = $ad[0][zone];
		$z = $this->db->getsql("SELECT * FROM adrev_zones WHERE id='$zoneid'");
		$zone = $z[0];
		...
		if(count($_REQUEST[bid]) > 0)
		{
			...
			while(list($xid,$rec) = each($_REQUEST[bid]))
			{
				...
				{
					$this->db->getsql("UPDATE adrev_keyword_map SET bid='$rec[bid]', url='$rec[url]'
										WHERE id='$xid' AND adid='$id'");
				}
		...
		{
			$kid = $_GET[delete];
			$this->db->getsql("DELETE FROM adrev_keyword_map WHERE adid='$id' AND keywordid='$kid'");
		}
		...
		{
			$num = $this->db->getsql("SELECT count(*) as num FROM adrev_keyword_map WHERE adid='$id'");
		...
		if($f[keyword])
		{
			...
			$exists = $this->db->getsql("SELECT id FROM adrev_keyword_map WHERE keywordid='$keywordid' AND adid='$id'");
			if(!$exists[0][id])
			{
				$i = array();
				$i[zoneid] = $zoneid;
		...
		$existing = $this->db->getsql("SELECT b.keyword,a.* FROM adrev_keyword_map a, adrev_keywords b
										WHERE a.keywordid=b.id AND a.adid='$id'
										ORDER BY b.keyword");
		if(count($existing) > 0)
		{
			$gen = new formgen();
			...
			foreach($existing as $rec)
			{
				...
				if($zone[rtype] == 2 && $zone[rate_type] == "CPC")
				{
					...
					$rates = $this->db->getsql("SELECT bid,adid FROM adrev_keyword_map 
												WHERE keywordid='$rec[keywordid]' AND zoneid='$zone[id]'
				...
				$stats = $this->db->getsql("SELECT count(id) as num, sum(clicks) as clicks, 
												...
												sum(amount) as spend
											FROM adrev_traffic
											WHERE adid='$id' AND userid='$uid' AND keywordid='$rec[keywordid]'
												AND date BETWEEN $startdate AND $enddate
											GROUP BY adid");					
		...
		$tpl->parse("main");
		$this->output->title = lib_lang("Manage Keywords");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;		
	...
	function stats()
	{
		...
		// Grab the ad info
		$ad = $this->db->getsql("SELECT * FROM adrev_ads WHERE zid='$f[id]'");
		$id = $ad[0][id];
		$zoneid = $ad[0][zone];
		
		// Grab the zone information
		$zone = $this->db->getsql("SELECT * FROM adrev_zones WHERE id='$zoneid'");
		...
		$stats = $this->db->getsql("SELECT * FROM adrev_traffic 
										WHERE adid='$id' AND keywordid='$f[kid]' 
										AND date BETWEEN $startdate AND $enddate
										ORDER BY date DESC
										LIMIT 100");
		if(count($stats) > 0)
		...
		if($f[kid])
		{
			$keywords = $this->db->getsql("SELECT b.id,b.keyword FROM adrev_keyword_map a, adrev_keywords b
											WHERE a.keywordid=b.id AND a.zoneid='$zoneid' AND a.adid='$id'
											ORDER BY b.keyword");
			$klist = array();
			if(count($keywords) > 0)
			{
		...
		$tpl->assign("ADS_MENU", $this->menu($_REQUEST[f][id]));
		$tpl->parse("main");
		$this->output->title = lib_lang("View Ad Stats");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;			
	...
	function preview()
	{
		...
		$this->output->title = lib_lang("Preview Ad");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;			
	...
	function schedule()
	{
		...
		$ad = $this->db->getsql("SELECT a.*,b.rate_type FROM adrev_ads a, adrev_zones b
									WHERE a.zid='$f[id]' AND a.userid='$uid' AND a.zone=b.id");
		...
		if($f[submit] && $ad[0][id])
		{
			...
			if($f[start_month] && $f[start_day] && $f[start_year])
				$f[startdate] = strtotime("$f[start_month]/$f[start_day]/$f[start_year] 0:0:1");
		...
		$tpl->assign("ADS_MENU", $this->menu($_REQUEST[f][id]));
		$tpl->parse("main");
		$this->output->title = lib_lang("Scheduling");
		$this->output->content = $tpl->text("main");
		$this->output->display();
		$this->output->printpage();		
		exit;
	...
	function menu($id="")
	{
		...
		$info = $this->db->getsql("SELECT a.title, a.status as ad_status, a.zid, b.* FROM adrev_ads a, adrev_zones b
									WHERE a.zone=b.id AND a.zid='$f[id]'");
